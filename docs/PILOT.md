...
- [ ] `npx supabase db push` has applied **every** migration (last one: `20260925100000_update_my_phone`); `npx supabase functions deploy` is done; secrets are set (`ALLOWED_ORIGIN`, `LOGIN_EMAIL_DOMAIN`, VAPID, `CRON_SECRET`).
