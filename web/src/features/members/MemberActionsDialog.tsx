...
import { deleteMember, memberNamesKey, membersKey, resetPassword, setMemberActive, updateMemberPhone, type Credentials } from './api';
...
type Pending = 'reset' | 'deactivate' | 'delete' | null;
...
export function MemberActionsDialog({ member, isSelf, onClose, onCredentials }: MemberActionsDialogProps) {
  ...
  const actionError = reset.error ?? toggle.error ?? remove.error;
        ...
        <>
          ...
          {isSelf ? null : pending === null ? (
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                {pending === 'reset' ? tr.members.resetConfirm : pending === 'deactivate' ? tr.members.deactivateConfirm : tr.members.deleteConfirm}
              </p>
              <div className="flex gap-2">
                ...
                <Button
                  variant={pending === 'reset' ? 'primary' : 'danger'}
                  className="flex-1"
                  loading={reset.isPending || toggle.isPending || remove.isPending}
                  onClick={() =>
                    pending === 'reset' ? reset.mutate(member.id) : pending === 'delete' ? remove.mutate(member.id) : toggle.mutate({ id: member.id, active: false })
                  }
                >
                  {pending === 'delete' ? tr.members.deleteConfirmAction : tr.members.confirmAction}
                </Button>
              </div>
            </div>
          )}
